...
export default function CategoryScreen() {
  ...
  useEffect(() => {
    const fetchCategoryProducts = async () => {
      try {
        ...
        const categoryRes = await fetch(`${process.env.EXPO_PUBLIC_API_URL2}products/group/`);
        const categoryData = await categoryRes.json();
        
        // Безопасно устанавливаем имя категории с проверкой на существование свойств
      if (categoryData && categoryData.description && categoryData.description.name) {
  setCategoryName(categoryData.description.name);
  ...
  setCategoryName(`Yza`);
        ...
        const productsRes = await fetch(`${process.env.EXPO_PUBLIC_API_URL2}products/group/short/${categoryid}?offset=0&max=100`);
  ...
  const renderProductItem = ({ item }: { item: Product }) => (
    ...
    >
      ...
      {item.newProduct && (
        ...
        {/* <Pressable className='absolute top-0 right-0 bg-[#5600B3] w-8 h-8 rounded-[5px] items-center justify-center outline-8 outline-white'
         onPress={() => AddBasketApi(item.id)}
      >
